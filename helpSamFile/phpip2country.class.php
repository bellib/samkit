...
class phpIp2Country {
	...
	private function dbConnect(){
		if(is_array($this->dbConfig)){
			...
			// }
			$this->db = mysql_connect($this->dbConfig['host'] , $this->dbConfig['dbUserName'], $this->dbConfig['dbUserPassword']);
			if (!$this->db) {
...
function SamGetInfoIp($ip)
{
		...
		'dbName' => 'netalarabi_com_sam_ip', //example db name
		'dbUserName' => 'saip_to_country', //example user name
		'dbUserPassword' => 'aazzee123', //example user password
